...
<div class="flex items-center justify-between mb-6">
  <div>
    <h1 class="text-xl font-bold text-gray-100">Mutasi Aset</h1>
...
<div class="card p-6 mb-6">
  <div class="flex items-center gap-2 mb-5">
    ...
    <h2 class="text-sm font-semibold text-gray-200">Catat Perpindahan Aset</h2>
  </div>
  ...
  <form method="POST" action="/ipsrs/aset/mutasi">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <!-- Lokasi Tujuan -->
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1.5">Lokasi Tujuan <span class="text-red-500">*</span></label>
      ...
      <!-- Alasan -->
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1.5">Alasan <span class="text-red-500">*</span></label>
      ...
      <!-- Status Baru -->
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1.5">Update Status Aset</label>
      ...
      <!-- Petugas -->
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1.5">Petugas <span class="text-red-500">*</span></label>
      ...
      <!-- Catatan -->
      <div>
        <label class="block text-xs font-semibold text-gray-300 mb-1.5">Catatan</label>
...
<div class="flex flex-wrap gap-2 mb-4">
  ...
  <a href="/ipsrs/aset/mutasi<?= $val ? '?alasan='.urlencode($val) : '' ?>"
     class="px-4 py-1.5 rounded-xl text-sm font-medium transition-colors <?= $active ? 'bg-[#CCFF00] text-black border-none text-white' : 'bg-[#121620]/60 text-gray-300 hover:bg-white/5 border border-white/10' ?>">
    <?= $label ?>
  </a>
  <?php endforeach; ?>
</div>
    ...
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y divide-gray-50">
        ...
        <tr class="hover:bg-gray-50/60 transition-colors">
          ...
          <td class="px-5 py-3.5 font-medium text-gray-100"><?= esc($r['nama_aset'] ?? $r['nama'] ?? '-') ?></td>
          <td class="px-5 py-3.5 text-gray-300"><?= esc($r['dari'] ?? $r['lokasi_asal'] ?? '-') ?></td>
          <td class="px-5 py-3.5 text-gray-300"><?= esc($r['ke'] ?? $r['lokasi_tujuan'] ?? '-') ?></td>
          <td class="px-5 py-3.5 text-gray-300"><?= esc($r['alasan'] ?? '-') ?></td>
          <td class="px-5 py-3.5 text-gray-300"><?= esc($r['petugas'] ?? '-') ?></td>
          <td class="px-5 py-3.5 text-gray-400 max-w-[180px] truncate"><?= esc($r['catatan'] ?? '-') ?></td>
        </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
