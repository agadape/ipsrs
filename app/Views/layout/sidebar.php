...
if (!function_exists('navLink')) {
    function navLink(string $href, string $label, string $icon, string $current, ?int $badge = null): string {
        ...
        return <<<HTML
        <a href="{$href}" onclick="closeSidebar()" class="flex items-center justify-between gap-3 px-3 py-2 rounded-md text-[13px] transition-colors duration-150 {$cls}">
...
if (!function_exists('ico')) {
    function ico(string $d, string $size = '16'): string {
        return "<svg xmlns='http://www.w3.org/2000/svg' width='{$size}' height='{$size}' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'>{$d}</svg>";
    }
}
?>

<aside id="sidebar" class="fixed left-0 top-0 h-screen w-64 flex flex-col z-30 -translate-x-full md:translate-x-0 bg-slate-50 border-r border-slate-200 transition-transform duration-200">
  ...
  <div class="flex items-center justify-between px-5 py-5">
    <div class="flex items-center gap-2.5">
      ...
      <div>
        <p class="text-slate-900 font-bold text-[14px] leading-none tracking-tight">IPSRS</p>
        <p class="text-slate-500 font-medium text-[10px] mt-1.5 leading-none uppercase tracking-wider">RSUD YK</p>
      </div>
    </div>
    <button onclick="closeSidebar()" class="md:hidden text-slate-400 hover:text-slate-700 bg-white border border-slate-200 p-1.5 rounded-md transition-colors">
  ...
  <div class="px-5 py-4 border-t border-slate-200 bg-slate-50">
    <div class="flex items-center gap-3">
      <div class="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-white text-[12px] font-medium shrink-0">
        <?= esc($authInitial) ?>
      </div>
      <div class="min-w-0">
      ...
      if (sidebar) {
        try {
          ...
          sidebar.addEventListener('scroll', function() {
            try { localStorage.setItem('sidebarScroll', sidebar.scrollTop); } catch(e){}
          });
        } catch(e){}
      }
    });
